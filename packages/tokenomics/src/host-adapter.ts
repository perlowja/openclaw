...
//! Codex, Cursor, OpenClaw — captures per-call usage in its own way (a hook, a
//! telemetry callback, a transcript). Each one normalizes that into a
//! {@link UsageEvent} and calls {@link ingest}, which writes a single ledger row.
//! The report/render layers then work identically regardless of host.
//!
...
export type HostId = "pi" | "claude-code" | "codex" | "cursor" | "openclaw" | string;
