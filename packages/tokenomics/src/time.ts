...
//! in one timezone rolls up identically everywhere (matches the Rust `chrono`
//! `Utc` formatting used by the companion Rust CLI).
 ...
 * week-year can differ from the calendar year near January/December, exactly as
 * `chrono::IsoWeek` produces (e.g. 2026-01-01 may fall in 2025-W53).
 */
...
export function parseDate(s: string, endOfDay = false): Date {
  ...
  const [, y, mo, da] = m;
  const t = endOfDay ? Date.UTC(+y, +mo - 1, +da, 23, 59, 59) : Date.UTC(+y, +mo - 1, +da, 0, 0, 0);
  return new Date(t);
}

/** Days between two dates (>= 1), matching `(until - since).num_days().max(1)`. */
